...
export function FinalMEL() {
  ...
  const handleFileUpload = async () => {
    ...
    try {
      const data = await uploadFinalMEL(file, cycle, year)
  ...
  const handlePascodeSubmit = async (sessionId: string, pascodeInfo: Record<string, any>) => {
    try {
      const blob = await submitFinalMELPascodes(sessionId, pascodeInfo)
      setPdfBlob(blob)
      setActiveStep(3)
    } catch (error: any) {
  ...
  const handleDownload = () => {
    if (pdfBlob) {
      const filename = `final_mel_${sessionData.session_id}.pdf`
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center space-x-3">
          <h2 className="text-3xl font-bold text-slate-900">Final Master Eligibility Listing</h2>
                ...
                <Card variant="bordered">
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      >
                        {isProcessing ? 'Processing Roster...' : 'Process Final MEL'}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
            ...
            <CardContent className="pt-6">
              <div className="text-center space-y-6">
                <div className="p-4 bg-success/10 border border-success rounded-lg">
                  ...
                  <p className="text-sm text-slate-700">
                    Your Final Master Eligibility Listing has been generated and is ready to download.
                  </p>
                </div>
